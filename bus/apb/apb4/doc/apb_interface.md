...
| 视图 | 类型 | 位置 |
|---|---|---|
| View A (packed struct) | `apb_pkg.sv` | `rtl/` |
| View B (SV interface) | `apb_if.sv` | `rtl/` |
| View C (flat wrapper) | `apb_flat_wrapper.sv` | `generated/apb/` |
| View D (spec md) | `apb_spec.md` | `generated/docs/apb/` |
| IP-XACT | `apb_busdef.xml` / `apb_absdef.xml` | `generated/ipxact/apb/` |
...
> 消费者示例仓库：`examples/apb_target/`（View A：`import apb_pkg::*`，使用 `apb_req_t`/`apb_rsp_t`）。
