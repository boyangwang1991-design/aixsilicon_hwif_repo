...
| 视图 | 类型 | 位置 |
|---|---|---|
| View A (packed struct) | `apb3_pkg.sv` | `rtl/` |
| View B (SV interface) | `apb3_if.sv` | `rtl/` |
| View C (flat wrapper) | `apb3_flat_wrapper.sv` | `generated/apb/` |
| View D (spec md) | `apb3_spec.md` | `generated/docs/apb/` |
| IP-XACT | `apb3_busdef.xml` / `apb3_absdef.xml` | `generated/ipxact/apb/` |
...
| `bus/apb/apb4`（`aixsilicon:interface:apb`） | `ADAPTER_REQUIRED` | APB3↔APB4 信号集不同（PSTRB/PPROT/PWAKEUP 缺失），需适配器补齐可选信号；不可直接连接 |
...
> 消费者示例仓库：`examples/apb3_target/`（View A：`import apb3_pkg::*`，使用 `apb3_req_t`/`apb3_rsp_t`）。
