...
| 视图 | 类型 | 位置 |
|---|---|---|
| View A (packed struct) | `ecc_memory_sideband_pkg.sv` | `rtl/` |
| View B (SV interface) | `ecc_memory_sideband_if.sv` | `rtl/` |
| View C (flat wrapper) | `ecc_memory_sideband_flat_wrapper.sv` | `generated/ecc_memory_sideband/` |
| View D (spec md) | `ecc_memory_sideband_spec.md` | `generated/docs/ecc_memory_sideband/` |
| IP-XACT | `ecc_memory_sideband_busdef.xml` / `ecc_memory_sideband_absdef.xml` | `generated/ipxact/ecc_memory_sideband/` |
...
```text
aixsilicon:interface:common → aixsilicon:interface:ecc_memory_sideband
```
...
ecc_memory_sideband_if #(
  .ADDR_W (32),
  .DATA_W (32)
) u_ecc_memory_sideband (
  .clk   (clk),
  .rst_n (rst_n)
);
```
