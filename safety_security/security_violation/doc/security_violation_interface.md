...
| 视图 | 类型 | 位置 |
|---|---|---|
| View A (packed struct) | `security_violation_pkg.sv` | `rtl/` |
| View B (SV interface) | `security_violation_if.sv` | `rtl/` |
| View C (flat wrapper) | `security_violation_flat_wrapper.sv` | `generated/security_violation/` |
| View D (spec md) | `security_violation_spec.md` | `generated/docs/security_violation/` |
| IP-XACT | `security_violation_busdef.xml` / `security_violation_absdef.xml` | `generated/ipxact/security_violation/` |
...
```text
aixsilicon:interface:common → aixsilicon:interface:security_violation
```
...
security_violation_if #(
  .ADDR_W (32),
  .DATA_W (32)
) u_security_violation (
  .clk   (clk),
  .rst_n (rst_n)
);
```
