...
def static_checks(family):
    ...
    source = folder / "contract" / f"{family}.interface.yaml"
    c = yaml.safe_load(source.read_text())
    name = c["interface"]["name"]
    Draft202012Validator(
        yaml.safe_load((ROOT / "schema/interface_contract.schema.yaml").read_text())
    ).validate(c)
    for p in folder.glob("contract/*.profile.yaml"):
        data = yaml.safe_load(p.read_text())
        Draft202012Validator(
            yaml.safe_load((ROOT / "schema/interface_profile.schema.yaml").read_text())
        ).validate(data)
    ...
    for field in ["inputs", "outputs"]:
        for rel, digest in provenance[field].items():
            assert sha(folder / rel) == digest, (family, "drift", rel)
    ...
    for t in core["targets"].values():
        assert set(t["filesets"]) <= set(core["filesets"])
    ...
    # Preserve repeated widths by keying declarations by signal name.
    ...
    for role in [r["id"] for r in c["roles"]]:
        ...
        actual = dict(
            (name, direction) for direction, name in re.findall(r"(input|output) (\w+)", block)
        )
...
def main():
    ...
    build = ROOT / "build/crypto_interfaces"
    build.mkdir(parents=True, exist_ok=True)
    config = build / "fusesoc.conf"
    config.write_text("[main]\n")
    smoke = subprocess.run(
        [sys.executable, str(Path(__file__).with_name("generate_smoke.py")), "--check-only"],
        check=False,
    )
    if smoke.returncode:
    ...
    for family in args.family or FAMILIES:
        ...
        if not args.static_only:
            ...
            cmd = [
                ...
                str(folder),
                "run",
                "--target=compile_smoke",
                "--build-root=" + str(build / family),
                "aixsilicon:interface:" + family + ":0.2.0",
            ]
            inputs = {
                str(p.relative_to(ROOT)): sha(p)
                for p in folder.rglob("*")
                if p.is_file() and p.suffix in [".sv", ".yaml", ".core"]
            }
            log = build / (family + ".log")
            with log.open("w") as output:
                try:
                    result = subprocess.run(
                        cmd, cwd=ROOT, stdout=output, stderr=subprocess.STDOUT, timeout=240
                    )
            ...
            passed = (
                code == 0
                and f"HWIF_SMOKE_PASS {family}" in text
                and text.count("PASS " + family + " DATA_W=") == 5
            )
        ...
        result_name = "results_static.json" if args.static_only else "results.json"
        (build / result_name).write_text(json.dumps(records, indent=2) + "\n")
    return int(
        any(not r.get("simulation_pass", True) or not r.get("inputs_stable", True) for r in records)
    )
