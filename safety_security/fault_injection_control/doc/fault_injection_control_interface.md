...
| 视图 | 类型 | 位置 |
|---|---|---|
| View A (packed struct) | `fault_injection_control_pkg.sv` | `rtl/` |
| View B (SV interface) | `fault_injection_control_if.sv` | `rtl/` |
| View C (flat wrapper) | `fault_injection_control_flat_wrapper.sv` | `generated/fault_injection_control/` |
| View D (spec md) | `fault_injection_control_spec.md` | `generated/docs/fault_injection_control/` |
| IP-XACT | `fault_injection_control_busdef.xml` / `fault_injection_control_absdef.xml` | `generated/ipxact/fault_injection_control/` |
...
```text
aixsilicon:interface:common → aixsilicon:interface:fault_injection_control
```
...
fault_injection_control_if #(
  .ADDR_W (32),
  .DATA_W (32)
) u_fault_injection_control (
  .clk   (clk),
  .rst_n (rst_n)
);
```
